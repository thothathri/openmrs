...
<script type="text/javascript">
	...
	$j(document).ready(function() {
		...
		hl7table = $j('#hl7Table').dataTable( { 
			"aoColumns": [  { "sName": "id", "bSortable": false, "bHidden": true},				
							{ "sName": "data", "bSortable": false,
					         "fnRender": function ( oObj ) {
								return '<div id="hl7' + oObj.iDataRow + '" class="showmore"><pre>' + oObj.aData[1] + 
									'</pre></div><a id="show'+ oObj.iDataRow + '" href=# onClick="showMore(' + oObj.iDataRow +
									')"><spring:message code="Hl7OutQueue.queueList.showMore" /></a>' +
									'<a id="hide'+ oObj.iDataRow + '" style="display:none;" href=# onClick="hideMore(' + oObj.iDataRow + 
									')"><spring:message code="Hl7OutQueue.queueList.hideMore" /></a>';
								}
							}
			  			 ],
			...
			"bServerSide": true,
			"bStateSave": false,
			"sAjaxSource": "hl7OutQueueList.json?messageState=${messageState}",
			"bJQueryUI": true,
			"oLanguage": {
...
<table cellpadding="5" cellspacing="0" id="hl7Table" width="100%">
	<thead>
		<tr>
			...
			<th><spring:message code="Hl7OutQueue.queueList.id" /></th>
			<th width="80%"><spring:message
				code="Hl7OutQueue.queueList.data" /></th>
		</tr>
	</thead>
	<tbody></tbody>
</table>
